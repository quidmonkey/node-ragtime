...
import { Corpus, batchCreateDatabases, createDatabases, readFiles, saveDatabases } from './utils/etl';
...
const downloadWikiContent = async (
  ...
  try {
    ...
    console.error('Unable to get Wikipedia content for', newWikiQuery.title, err);
  }
...
const getBooks = async (): Promise<Pick<Document, 'title' | 'url'>[]> => {
  ...
  return anchors
    ...
    .filter((anchor) => !IgnoredCollections.includes(anchor.textContent as string))
    .map((anchor) => ({
...
Program
  .name('create-sherlock-corpus')
  .description('Create Sherlock Holmes corpus')
  .option('-b, --batch', 'Batch create vector databases - Use if GPU is available')
