...
const answerQuery = async (query: string, texts: string[]): Promise<void> => {
	const results = texts.map((text, i) => `${i + 1}. ${text}`).join("\n\n");
...
const main = async (question: string) => {
	...
	const urls = searchRes
		.map(({ link }) => link)
		.filter((url) => {
			const IgnoredDomains = ["quora.com", "reddit.com", "nytimes.com"];
...
Program.description("Get answers to all your life questions!")
